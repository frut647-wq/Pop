...
# ------------------------------------------------------------------
# دامداری‌ها — اول ساختمون، بعد حیوان جدا. تولید مداوم (نرخ در ساعت) با سقف بافر
# ------------------------------------------------------------------
LIVESTOCK_BUILDINGS = {
    "مرغداری": {
...
# ------------------------------------------------------------------
# مزرعه‌ها — اول زمین، بعد هر بار کاشت با سکه مستقیم داخل همون کارخونه
# ------------------------------------------------------------------
FARM_BUILDINGS = {
...
# ------------------------------------------------------------------
# کارخونه‌های پردازش — هرکدوم مخصوص یه محصول فرعیه، ورودیش فقط از تولید خودته
# ------------------------------------------------------------------
FACTORY_BUILDINGS = {
...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS livestock_state (
            ...
            buffer_qty REAL DEFAULT 0,
            last_update_ts REAL DEFAULT 0,
            PRIMARY KEY (user_id, building_key)
        )
    """)
    c.execute("""
...
def buy_building(user_id: int, building_key: str):
    ...
    if info["kind"] == "livestock":
        c.execute(
            "INSERT INTO livestock_state (user_id, building_key, animal_count, buffer_qty, last_update_ts) VALUES (?, ?, 0, 0, ?)",
...
def collect_livestock(user_id: int, building_key: str):
    sync_livestock(user_id, building_key)
    info = LIVESTOCK_BUILDINGS[building_key]
    state = get_livestock_state(user_id, building_key)
    ...
    conn = db()
    c = conn.cursor()
    c.execute("UPDATE livestock_state SET buffer_qty = buffer_qty - ? WHERE user_id=? AND building_key=?", (added, user_id, building_key))
    conn.commit()
    conn.close()
    return added
...
def nav_row_button(cb):
    labels = {"menu:factory": "🏭 کارخونه", "menu:farm": "🌱 مزرعه", "menu:livestock": "🥚 دامداری"}
    return InlineKeyboardButton(labels[cb], callback_data=cb)


def build_farm_panel(user_id: int):
    owned = get_owned_buildings(user_id)
    lines = ["🌱—مزرعه‌های من—🌱", ""]
    buttons = []
    now = time.time()
    has_any = False
    for key, info in FARM_BUILDINGS.items():
        if key not in owned:
            continue
        has_any = True
        level = owned[key]
        ...
        if job:
            ...
            if left <= 0:
                lines.append(f"✅ {info['emoji']} {label} (سطح {level}) — آماده برداشت! ({info['yield_qty']} {info['crop']})")
                buttons.append([InlineKeyboardButton(f"برداشت {label}", callback_data=f"farmharvest:{job['id']}")])
            else:
                lines.append(f"⏰ {fmt_countdown(left)} <-- {info['emoji']} {label} (سطح {level})")
        else:
            lines.append(f"{info['emoji']} {label} (سطح {level}) — کاشت با {info['plant_cost']} سکه، {info['plant_seconds']//60} دقیقه")
            buttons.append([InlineKeyboardButton(f"کاشت {label} ({info['plant_cost']} سکه)", callback_data=f"farmstart:{key}")])
        buttons.append([InlineKeyboardButton(f"⬆️ ارتقای {label}", callback_data=f"upgrade:{key}")])

    if not has_any:
        lines.append("هنوز هیچ زمین مزرعه‌ای نخریدی.")
    buttons.append([InlineKeyboardButton("🛒 خرید زمین مزرعه جدید", callback_data="shopcat:farm")])
    buttons.append([nav_row_button("menu:factory"), nav_row_button("menu:farm"), nav_row_button("menu:livestock")])
    return "\n".join(lines), InlineKeyboardMarkup(buttons)


def build_livestock_panel(user_id: int):
    owned = get_owned_buildings(user_id)
    lines = ["🥚—دامداری‌های من—🥚", ""]
    buttons = []
    has_any = False
    for key, info in LIVESTOCK_BUILDINGS.items():
        if key not in owned:
            continue
        has_any = True
        ...
        lines.append(
            f"{info['emoji']} {label} (سطح {level})\n"
            f"  تعداد {info['animal_key']}: {state['animal_count']}/{max_animals}\n"
            f"  نرخ تولید: {rate_per_hour:.2f} {info['output']} در ساعت\n"
            f"  بافر: {int(state['buffer_qty'])}/{buffer_cap}"
        )
        buttons.append([InlineKeyboardButton(f"➕ خرید {info['animal_key']} ({info['animal_cost']} سکه)", callback_data=f"buyanimal:{key}")])
        if state["buffer_qty"] >= 1:
            buttons.append([InlineKeyboardButton(f"📥 جمع‌آوری {info['output_emoji']} {building_label(info['output'])}", callback_data=f"livecollect:{key}")])
        buttons.append([InlineKeyboardButton(f"⬆️ ارتقای {label}", callback_data=f"upgrade:{key}")])

    if not has_any:
        lines.append("هنوز هیچ دامداری‌ای نخریدی.")
    buttons.append([InlineKeyboardButton("🛒 خرید دامداری جدید", callback_data="shopcat:livestock")])
    buttons.append([nav_row_button("menu:factory"), nav_row_button("menu:farm"), nav_row_button("menu:livestock")])
    return "\n".join(lines), InlineKeyboardMarkup(buttons)


def build_factory_panel(user_id: int):
    owned = get_owned_buildings(user_id)
    inv = get_inventory(user_id)
    ...
    now = time.time()
    has_any = False
    for key, info in FACTORY_BUILDINGS.items():
        if key not in owned:
            continue
        has_any = True
        level = owned[key]
        ...
        if job:
            left = job["ready_at"] - now
            if left <= 0:
                lines.append(f"✅ {info['emoji']} {label} (سطح {level}) — آماده برداشت! ({info['output_qty']} {info['output']})")
                buttons.append([InlineKeyboardButton(f"برداشت {label}", callback_data=f"factharvest:{job['id']}")])
            else:
                lines.append(f"⏰ {fmt_countdown(left)} <-- {info['emoji']} {label} (سطح {level})")
        else:
            have = inv.get(info["input"], 0)
            lines.append(f"{info['emoji']} {label} (سطح {level}) — نیاز {info['input_qty']} {info['input']} (داری: {have})")
            buttons.append([InlineKeyboardButton(f"تولید {label}", callback_data=f"factstart:{key}")])
        buttons.append([InlineKeyboardButton(f"⬆️ ارتقای {label}", callback_data=f"upgrade:{key}")])

    if not has_any:
        lines.append("هنوز هیچ کارخونه‌ای نخریدی.")
    buttons.append([InlineKeyboardButton("🛒 خرید کارخونه جدید", callback_data="shopcat:factory")])
    buttons.append([nav_row_button("menu:factory"), nav_row_button("menu:farm"), nav_row_button("menu:livestock")])
    return "\n".join(lines), InlineKeyboardMarkup(buttons)
...
def build_shop_category_view(user_id: int, kind: str):
    registry = {"livestock": LIVESTOCK_BUILDINGS, "farm": FARM_BUILDINGS, "factory": FACTORY_BUILDINGS}[kind]
    title = {"livestock": "دامداری‌ها", "farm": "زمین‌های مزرعه", "factory": "کارخونه‌های پردازش"}[kind]
    owned = get_owned_buildings(user_id)
    lines = [f"🛒 خرید {title}", ""]
    buttons = []
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    guide = (
        ...
        "📖 قوانین بازی:\n"
        "🐔 دامداری — اول ساختمون (مثلاً مرغداری) رو از شاپ بخر، بعد جدا حیوان (مرغ) بخر و بهش اضافه کن. "
        "تولید مداومه (بر اساس تعداد حیوون در ساعت) و یه سقف بافر داره؛ باید مرتب جمع‌آوری کنی.\n"
        "🌾 مزرعه — اول زمین رو از شاپ بخر. برای کاشت هر بار مستقیم سکه میدی (نه خرید دونه)؛ بعد از یه مدت برداشت می‌کنی و با فروش ۳-۴ برابر سکه‌ای که دادی برمی‌گردونه.\n"
        "🏭 کارخونه پردازش — هر محصول فرعی (نان، نیمرو، پارچه، ماست...) کارخونه اختصاصی خودشو لازم داره؛ باید جدا بخریش. ورودیش فقط از تولید خودته.\n"
...
async def production_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    ensure_player(user.id, user.username or user.first_name)
    text, kb = build_farm_panel(user.id)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:livestock":
        text, kb = build_livestock_panel(user.id)
        await query.edit_message_text(text, reply_markup=kb)
        return
    if data == "menu:factory":
        text, kb = build_factory_panel(user.id)
        await query.edit_message_text(text, reply_markup=kb)
        return
    ...
    if data.startswith("upgrade:"):
        ...
        if not ok:
            ...
            return
        kind = ALL_BUILDINGS[key]["kind"]
        panel_fn = {"farm": build_farm_panel, "livestock": build_livestock_panel, "factory": build_factory_panel}[kind]
        text, kb = panel_fn(user.id)
        await query.edit_message_text(text + f"\n\n✅ {msg}", reply_markup=kb)
        return
