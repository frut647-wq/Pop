...
پس از فاز ۱۳: بازتعادل کامل اقتصاد، منابع نامحدود ادمین، معدن‌های رایگان، گرسنگی واقعی با کف جمعیت،
نیروی دائمی کارخونه، گزارش تولید ساعتی، خرید حداکثر تو شاپ‌ها، انتخاب/تغییر اسم کشور، دکمه ثابت شروع بازی
"""
...
DEFAULT_SETTINGS = {
    ...
    # هر کارخونه به‌جز کارگرهای تخصیص‌داده‌شده، یه «نیروی دائمی» رایگان هم داره که هیچ‌وقت نمی‌خوابه
    "factory_permanent_worker_bonus": "1",
    ...
    "power_industrial_build_gold": "3000",
    ...
    "defense_upgrade_base_gold": "1200",
    "defense_upgrade_base_metal": "100",

    "defense_repair_gold_per_tier": "500",
    ...
    # ===== فاز ۸: شاپ‌ها =====
    # شاپ ادمین: قیمت خرید هر واحد منبع (با پول) - همیشه زیر ۱۰۰۰ چون واحدی هستن
    "admin_shop_oil_price": "15",
    "admin_shop_metal_price": "25",
    "admin_shop_fertilizer_price": "8",
    "admin_shop_electricity_price": "12",
    ...
    # فروش فوری: ارزش پایه هر واحد نظامی (قبل از تقسیم بر نسبت فروش فوری) - بالای ۳۰۰۰ طبق درخواست
    "instant_sell_unit_value_tank": "3200",
    "instant_sell_unit_value_jet": "5500",
    "instant_sell_unit_value_ship": "7500",
    "instant_sell_unit_value_missile": "9000",
    ...
    "research_upgrade_base_gold": "1200",
    ...
    # ===== سرمایه اولیه بازیکن (قابل تنظیم توسط ادمین) =====
    "starter_gold": "1500",
    "starter_oil": "150",
    "starter_metal": "150",
    "starter_fertilizer": "100",
    "starter_food": "100",
    "starter_electricity_civil": "80",

    # ===== حداقل جمعیت باقی‌مونده (هیچ‌وقت کشور کامل نابود نمی‌شه) =====
    "min_population_floor": "10",
    "starvation_loss_rate_per_hour": "2",   # چند نفر بیکار در ساعت گرسنگی می‌کشن اگه غذا صفر باشه
}
...
def create_new_player(user_id: int, referrer_id: int = None):
    ...
    starter_gold = 999999999 if admin_flag else int(get_setting("starter_gold") or 1500)
    ...
    """, (user_id, country_name, admin_flag, referrer_id, join_date,
          protection_until, 10, starter_gold, 0, join_date))

    if admin_flag:
        # ادمین منابع نامحدود داره (سقف‌ها هم خیلی بالا می‌ره که هیچ‌وقت پر نشه)
        big = 999999999
        cur.execute("""INSERT INTO resources
            (user_id, fertilizer, fertilizer_cap, food, food_cap, oil, oil_cap, metal, metal_cap,
             electricity_civil, electricity_civil_cap, electricity_industrial, electricity_industrial_cap,
             fertilizer_sellable, oil_sellable, metal_sellable, last_production_check)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1, 1, ?)""",
            (user_id, big, big, big, big, big, big, big, big, big, big, big, big, join_date))
    else:
        starter_oil = int(get_setting("starter_oil") or 150)
        starter_metal = int(get_setting("starter_metal") or 150)
        starter_fertilizer = int(get_setting("starter_fertilizer") or 100)
        starter_food = int(get_setting("starter_food") or 100)
        starter_elec = int(get_setting("starter_electricity_civil") or 80)
        cur.execute("""INSERT INTO resources
            (user_id, fertilizer, food, oil, metal, electricity_civil, last_production_check)
            VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (user_id, starter_fertilizer, starter_food, starter_oil, starter_metal, starter_elec, join_date))

    cur.execute("""INSERT INTO population (user_id, last_growth_check)
                    VALUES (?, ?)""", (user_id, join_date))

    # ساختمان‌های اولیه: ۲ خونه، ۱ مزرعه، هر سه معدن (کود/نفت/فلز) رایگان، ۱ نیروگاه عادی
...
def calculate_and_apply_production(user_id: int):
    ...
    pre_consumption_food = min(res["food"] + food_produced, res["food_cap"])
    ...
    new_food = max(0, pre_consumption_food - food_consumed)
    food_shortfall = max(0, food_consumed - pre_consumption_food)

    # ---- گرسنگی: اگه غذا کم بیاد، از جمعیت بیکار کم می‌شه (هیچ‌وقت زیر حداقل مجاز نمی‌ره) ----
    new_unemployed = pop["unemployed"]
    if food_shortfall > 0 and food_consumed > 0:
        starvation_rate = float(get_setting("starvation_loss_rate_per_hour") or 2)
        floor = int(get_setting("min_population_floor") or 10)
        shortage_ratio = min(1.0, food_shortfall / food_consumed)
        starvation_loss = round(starvation_rate * elapsed_hours * shortage_ratio)
        room_above_floor = max(0, total_population - floor)
        actual_loss = min(starvation_loss, new_unemployed, room_above_floor)
        new_unemployed = max(0, new_unemployed - actual_loss)
    ...
    if growth_elapsed_hours >= 24:
        if new_food > 0:
            current_total_pop = (new_unemployed + pop["miners_fertilizer"] + pop["miners_oil"] +
                                  pop["miners_metal"] + pop["factory_workers"] + pop["military_crew"])
    ...
    if new_unemployed != pop["unemployed"] or new_last_growth_check != pop["last_growth_check"]:
    ...
    if factories:
        permanent_bonus = float(get_setting("factory_permanent_worker_bonus") or 1)
        workers_per_factory = (pop["factory_workers"] / len(factories)) + permanent_bonus
...
def profit_summary_text(user_id: int) -> str:
    """گزارش تولید/مصرف ساعتی هر بخش - بدون تغییر دادن دیتابیس"""
    calculate_and_apply_production(user_id)  # اول منابع رو به‌روز کن که گزارش دقیق باشه
    ...
    oil_bonus = 1 + get_research_bonus_percent(user_id, "oil_refine") / 100
    metal_bonus = 1 + get_research_bonus_percent(user_id, "metal") / 100
    food_bonus = 1 + get_research_bonus_percent(user_id, "food") / 100
    ...
    fert_per_hour = pop["miners_fertilizer"] * fert_rate * (1 + 0.1 * max(0, fert_level - 1))
    oil_per_hour = pop["miners_oil"] * oil_rate * (1 + 0.1 * max(0, oil_level - 1)) * oil_bonus
    metal_per_hour = pop["miners_metal"] * metal_rate * (1 + 0.1 * max(0, metal_level - 1)) * metal_bonus
    food_per_hour = farm_level * farm_food_rate * food_bonus
    fert_consumed_by_farm = farm_level * farm_fert_consumption
    elec_civil_per_hour = len(b_by_type.get("power_civil", [])) * power_output_rate
    elec_ind_per_hour = len(b_by_type.get("power_industrial", [])) * nuclear_output_rate

    total_pop = (pop["unemployed"] + pop["miners_fertilizer"] + pop["miners_oil"] +
                 pop["miners_metal"] + pop["factory_workers"] + pop["military_crew"])
    food_consumed_per_hour = total_pop * food_consumption_per_hour

    text = (
        "📈 *گزارش تولید و مصرف ساعتی*\n\n"
        f"💩 کود: تولید +{fert_per_hour:.1f}/ساعت | مصرف مزرعه -{fert_consumed_by_farm:.1f}/ساعت\n"
        f"🛢️ نفت: تولید +{oil_per_hour:.1f}/ساعت\n"
        f"⚙️ فلز: تولید +{metal_per_hour:.1f}/ساعت\n"
        f"🌾 غذا: تولید +{food_per_hour:.1f}/ساعت | مصرف جمعیت -{food_consumed_per_hour:.1f}/ساعت\n"
        f"⚡ برق عادی: +{elec_civil_per_hour:.1f}/ساعت\n"
        f"☢️ برق صنعتی: +{elec_ind_per_hour:.1f}/ساعت\n\n"
    )
    ...
    if factories:
        ...
        text += "🏭 *کارخونه‌ها (ساعتی)*\n"
        for f in factories:
            item_code, item_type, damage, crew = get_factory_active_model(user_id, f)
            units_per_hour = workers_per_factory * production_rate * f["level"]
            metal_used = units_per_hour * metal_cost_per_unit
            elec_used = units_per_hour * elec_cost_per_unit
            text += (f"  {item_code} (سطح {f['level']}): +{units_per_hour:.2f} واحد | "
                     f"مصرف فلز -{metal_used:.1f} | مصرف برق صنعتی -{elec_used:.1f}\n")
...
def country_menu_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📈 گزارش تولید ساعتی", callback_data="country_profit")],
        [InlineKeyboardButton("✏️ تغییر اسم کشور", callback_data="country_rename")],
...
def build_menu_keyboard(user_id: int):
    ...
    owned_mines = {r["type"] for r in conn.execute(
        "SELECT type FROM buildings WHERE user_id=? AND type IN ('mine_metal','mine_fertilizer','mine_oil')",
        (user_id,)).fetchall()}
    conn.close()
    ...
    mine_defs = [("mine_fertilizer", "💩 معدن کود"), ("mine_oil", "🛢️ معدن نفت"), ("mine_metal", "⚙️ معدن فلز")]
    for mine_type, label in mine_defs:
        if mine_type in owned_mines:
            buttons.append([InlineKeyboardButton(f"⬆️ ارتقای {label}", callback_data=f"upgrade_{mine_type}")])
        else:
            buttons.append([InlineKeyboardButton(f"🏗️ ساخت {label}", callback_data=f"build_{mine_type}")])
...
def get_action_cost(user_id: int, action_key: str, quantity: int):
    ...
    if action_key == "build_mine_fertilizer":
        g = int(get_setting("mine_fertilizer_build_gold") or 1000)
        return {"gold": g * quantity}

    if action_key == "build_mine_oil":
        g = int(get_setting("mine_oil_build_gold") or 1500)
        return {"gold": g * quantity}
...
def apply_action(user_id: int, action_key: str, quantity: int):
    ...
    elif action_key == "build_mine_fertilizer":
        conn.execute("""INSERT INTO buildings (user_id, type, level, hp, built_at)
                         VALUES (?, 'mine_fertilizer', 1, 0, ?)""", (user_id, now_str()))

    elif action_key == "build_mine_oil":
        conn.execute("""INSERT INTO buildings (user_id, type, level, hp, built_at)
                         VALUES (?, 'mine_oil', 1, 0, ?)""", (user_id, now_str()))
...
ACTION_LABELS = {
    ...
    "build_mine_fertilizer": "ساخت معدن کود",
    "build_mine_oil": "ساخت معدن نفت",
...
def population_status_text(user_id: int) -> str:
    conn = get_db()
    pop = conn.execute("SELECT * FROM population WHERE user_id=?", (user_id,)).fetchone()
    mine_levels = {}
    for b_type in ("mine_fertilizer", "mine_oil", "mine_metal"):
        row = conn.execute("SELECT level FROM buildings WHERE user_id=? AND type=?",
                            (user_id, b_type)).fetchone()
        mine_levels[b_type] = row["level"] if row else 0
    ...
    permanent_bonus = get_setting("factory_permanent_worker_bonus") or "1"

    fert_cap = mine_levels["mine_fertilizer"] * cap_per_level
    oil_cap = mine_levels["mine_oil"] * cap_per_level
    metal_cap = mine_levels["mine_metal"] * cap_per_level

    text = (
        "👥 *تخصیص جمعیت*\n\n"
        f"🔸 بیکار: {pop['unemployed']}\n"
        f"🔸 کارگر معدن کود: {pop['miners_fertilizer']}/{fert_cap} (ظرفیت بر اساس سطح معدن)\n"
        f"🔸 کارگر معدن نفت: {pop['miners_oil']}/{oil_cap}\n"
        f"🔸 کارگر معدن فلز: {pop['miners_metal']}/{metal_cap}\n"
        f"🔸 کارگر کارخونه: {pop['factory_workers']} (بدون سقف؛ بین {factory_count} کارخونه تقسیم می‌شه، "
        f"هر کارخونه {permanent_bonus} نیروی رایگان دائمی هم داره)\n"
        f"🔸 خدمه نظامی: {pop['military_crew']}\n\n"
        "💡 برای افزایش ظرفیت کارگر معدن، سطح همون معدن رو ارتقا بده.\n\n"
...
# ============================================================
START_BUTTON_TEXT = "🎮 شروع بازی"
PERSISTENT_KEYBOARD = ReplyKeyboardMarkup([[KeyboardButton(START_BUTTON_TEXT)]], resize_keyboard=True)
...
def main_menu_keyboard(user_id: int):
    buttons = [
        ...
        [InlineKeyboardButton("📢 اعلامیه به کانال", callback_data="menu_announce")],
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not player_exists(user_id):
        ...
        welcome_text = (
            ...
            "کشور تازه‌ت ساخته شد و یه سری سرمایه اولیه گرفتی 💰🏠🌾💩🛢️⚙️🛡️\n\n"
            "اول از همه، اسم کشورت رو بنویس و بفرست (اختیاریه، بعداً هم می‌تونی از منو عوضش کنی):"
        )
        set_pending_action(user_id, "set_country_name", {"first_time": True})
...
HELP_SECTIONS = {
    "start": ("🚀 چطوری شروع کنم؟", (
        "🚀 *شروع بازی*\n\n"
        "۱. از اول یه مقدار پول و منابع (نفت، فلز، کود، غذا، برق) و ۲ خونه و یه مزرعه و هر سه معدن داری\n"
        "۲. اول برو «کشور من» ← «تخصیص جمعیت» و چندتا از بیکارها رو بفرست کارگری معدن‌ها "
        "(کود/نفت/فلز) تا تولید شروع بشه\n"
        "۳. مزرعه بدون کارگر هم غذا تولید می‌کنه، نگرانش نباش\n"
        "۴. وقتی فلز و پول جمع شد، از «کارخونه‌های نظامی» یه کارخونه (مثلاً تانک) بساز و یکی از "
        "کارگرهات رو بفرست اونجا\n"
        "۵. با گذشت زمان، کارخونه خودش (حتی بدون کارگر هم یه‌کم) واحد نظامی می‌سازه؛ از «حمله» "
        "می‌تونی به یه کشور هم‌سطح خودت حمله کنی\n"
        "۶. یه پدافند پایه از اول داری؛ برای امنیت بیشتر ارتقاش بده\n"
        "۷. به یه اتحاد بپیوند یا بسازش تا از گیفت و پشتیبانی گروهی بهره ببری\n\n"
        "هر وقت گیر کردی، بقیه بخش‌های همین راهنما رو بخون 👇"
    )),
    "country": ("🏙️ کشور من", (
        "🏙️ *کشور من*\n\n"
        "از منوی «کشور من» می‌تونی:\n"
        "🏗️ *ساختمان‌سازی*: خونه، نیروگاه عادی/اتمی، معدن فلز بسازی یا مزرعه/معادن رو ارتقا بدی\n"
        "📏 *ارتقای طول کشور*: هر ارتقا ۱۰ جای خونه جدید اضافه می‌کنه\n"
        "👥 *تخصیص جمعیت*: جمعیت بیکار رو بفرست سراغ کارگری معدن‌ها یا کارخونه\n"
        "📈 *گزارش تولید ساعتی*: می‌بینی هر بخش چقدر تولید/مصرف می‌کنه\n\n"
        "💡 موقع ساخت/ارتقا، دو گزینه داری: «تعداد دلخواه» (عدد رو تایپ می‌کنی) یا «حداکثر ممکن» "
        "(خودش بیشترین مقدار قابل‌پرداخت رو حساب می‌کنه).\n\n"
        "منابع (کود، نفت، فلز، برق، غذا) خودکار و به‌مرور تولید می‌شن؛ هر بار وارد «کشور من» بشی، "
        "تولید از آخرین بار محاسبه می‌شه."
    )),
    "military": ("🏭 کارخونه و پدافند", (
        "🏭 *کارخونه‌های نظامی*\n\n"
        "از «کشور من» ← «کارخونه‌های نظامی» می‌تونی کارخونه تانک/جنگنده/کشتی/موشک بسازی. "
        "هر کارخونه بدون نقشه، یه مدل پایه می‌سازه؛ با خرید نقشه از شاپ نقشه‌ساخت و نصبش، مدل بهتر می‌سازه.\n"
        "کارگر کارخونه (از تخصیص جمعیت) لازمه تا تولید انجام بشه.\n\n"
        "🛡️ *پدافند*\n\n"
        "۴ تیر پدافند داری (سبک تا فوق‌سنگین)، هرکدوم درصدی از دمیج ورودی رو دفع می‌کنه. "
        "بعد از چند بار دفاع، نیاز به تعمیر پیدا می‌کنه (چند ساعت زمان می‌بره و پول لازم داره)."
    )),
    "attack": ("⚔️ حمله", (
        "⚔️ *حمله*\n\n"
        "۱. از منوی «حمله» واحد نظامی موردنظرت رو انتخاب و تعدادش رو بگو (می‌تونی چند نوع مختلف اضافه کنی)\n"
        "۲. یه هدف انتخاب کن (فقط کشورهایی که تفاوت قدرتشون باهات کمتر از حد مجازه مستقیم قابل‌انتخابن؛ "
        "بقیه نیاز به تایید ادمین دارن)\n"
        "۳. حمله ثبت می‌شه و واحدها همون لحظه مصرف می‌شن، ولی نتیجه (بعد از محاسبه پدافند حریف) "
        "چند ساعت بعد بهت اعلام می‌شه\n\n"
        "غنیمت جنگی به‌صورت *کاپ* (نه منابع مستقیم) به برنده می‌رسه."
    )),
    "alliance": ("🤝 اتحاد و جنگ اتحادی", (
        "🤝 *اتحاد*\n\n"
        "می‌تونی اتحاد بسازی یا به یکی بپیوندی. رهبر می‌تونه عضو اخراج کنه، همه اعضا می‌تونن به هم "
        "گیفت بدن (حداکثر ۵۰٪ دارایی، هر ۱۲ ساعت یه‌بار) یا به گاوصندوق مشترک واریز کنن.\n\n"
        "📢 هر عضوی می‌تونه اعلامیه به کانال بفرسته؛ فقط رهبر می‌تونه انتخاب کنه به نام اتحاد یا کشور خودش.\n\n"
        "⚔️ *جنگ اتحادی (قبیله‌ای)*\n\n"
        "رهبر یه اتحاد دیگه رو هدف می‌گیره، اعضا رأی می‌دن، و بعد از تایید اکثریت و شروع ادمین، "
        "تجهیزات اهداشده به انبار اتحاد علیه ضعیف‌ترین عضو حریف اول استفاده می‌شه. "
        "کشور نابودشده ۲۰٪ ثروتش رو از دست می‌ده و ۱۲ ساعت سپر می‌گیره."
    )),
    "shops": ("🏪 خرید و فروش", (
        "🏪 *شاپ‌ها*\n\n"
        "🏦 *شاپ ادمین*: نفت/فلز/کود/برق رو با پول می‌خری\n"
        "🏅 *شاپ کاپ*: فلز یا آیتم ویژه رو با کاپ می‌خری\n"
        "📜 *شاپ نقشه‌ساخت*: نقشه مدل‌های پیشرفته‌تر رو با پول می‌خری، بعد از «نقشه‌های من» نصبش می‌کنی\n\n"
        "🛒 *بازار بازیکنان (فروش به بقیه)*:\n"
        "۱. از شاپ‌ها ← «بازار بازیکنان» ← «ثبت آگهی فروش»\n"
        "۲. دسته موردنظر (منبع یا مدل نظامی) رو انتخاب کن\n"
        "۳. تعداد رو تایپ کن (موجودی فعلیت تو پیام نشون داده می‌شه)\n"
        "۴. قیمت هر واحد رو بنویس\n"
        "۵. یه کد یکتا (مثل T5414) بهت داده می‌شه؛ بقیه با دیدن اون کد تو لیست می‌تونن بخرنش\n\n"
        "⚡ *فروش فوری*: هرچی داری (منابع یا تجهیزات نظامی که ساخته باشی) رو با ۱/۴ قیمت بازار فوری "
        "به سیستم می‌فروشی. اگه چیزی تو لیست نیست یعنی الان ازش صفر داری."
    )),
    "lab": ("🔬 آزمایشگاه", (
        "🔬 *آزمایشگاه*\n\n"
        "۴ خط تحقیق داری: زره (تقویت پدافند)، پالایش نفت، فرآوری فلز، بهبود کشاورزی. "
        "هرکدوم تا سطح ۱۰ ارتقا پیدا می‌کنه و مستقیم رو نرخ دفاع یا تولید منابعت اثر می‌ذاره."
    )),
}


def help_categories_keyboard():
    buttons = [[InlineKeyboardButton(label, callback_data=f"help_sec_{key}")]
               for key, (label, _) in HELP_SECTIONS.items()]
...
ADMIN_HELP_TEXT = (
    "👑 *راهنمای ادمین*\n\n"
    "⚙️ *تنظیم قیمت‌ها*: پنل ادمین ← «تنظیم قیمت‌ها» ← یه دسته انتخاب کن ← رو تنظیم موردنظر بزن ← "
    "عدد جدید رو بفرست. همه‌چیز (قیمت‌ ساخت، دمیج، اچ‌پی، نرخ پدافند، هزینه‌های شاپ و...) از همینجا قابل‌تغییره.\n\n"
    "⚔️ *تایید حمله*: حمله‌های خارج از محدوده قدرت میان پیش تو؛ از «حمله‌های در انتظار تایید» تایید/رد کن. "
    "اگه ۱۲ ساعت جواب ندی، خودکار بر اساس تفاوت قدرت فعلی تصمیم گرفته می‌شه.\n\n"
    "🤝 *مدیریت اتحادها*: از پنل، هر عضوی از هر اتحادی رو می‌تونی اخراج کنی.\n\n"
    "📊 *آمار*: تعداد کاربران، اتحادها، جنگ‌ها و اعلامیه‌های امروز رو می‌بینی.\n\n"
    "🏳️ برای شروع، اول از منوی «اتحاد» یه اتحاد برای خودت بساز؛ خودکار به‌عنوان اتحاد ادمین (عضویت نامحدود، "
    "غیرقابل‌حمله) شناخته می‌شه و تازه‌واردها اولش میرن توش."
)


async def myid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """دستور کمکی برای دیباگ: فقط آیدی خودت رو نشون می‌ده. مقدار ADMIN_ID فقط به خود ادمین نشون داده می‌شه."""
    user_id = update.effective_user.id
    is_you_admin = is_admin(user_id)
    if is_you_admin:
        text = f"🆔 آیدی عددی تو: `{user_id}`\n✅ ادمینی، همه‌چیز درسته."
    else:
        text = f"🆔 آیدی عددی تو: `{user_id}`\n❌ این آیدی با ADMIN_ID تنظیم‌شده یکی نیست."
...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("help_sec_"):
        key = query.data.replace("help_sec_", "")
        if key not in HELP_SECTIONS:
            return
        _, text = HELP_SECTIONS[key]
        back_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="menu_help")]])
        await query.edit_message_text(text, reply_markup=back_btn, parse_mode="Markdown")
        return

    if query.data == "help_admin":
        back_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="menu_admin")]])
        await query.edit_message_text(ADMIN_HELP_TEXT, reply_markup=back_btn, parse_mode="Markdown")
        return
    ...
    if query.data == "menu_announce":
        if not ANNOUNCEMENT_CHANNEL_ID:
            await query.edit_message_text(
                "❌ کانال اعلامیه هنوز توسط ادمین تنظیم نشده.",
        ...
        if a and a["leader_id"] == user_id:
            ...
            await query.edit_message_text("📢 متن پیامت رو بنویس تا تو کانال منتشر بشه (به نام کشور خودت):")
        return
...
async def country_build_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "country_population":
        await query.edit_message_text(
            population_status_text(user_id),
            reply_markup=population_menu_keyboard(),
            parse_mode="Markdown"
        )
        return

    if query.data == "country_profit":
        back_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="menu_country")]])
        await query.edit_message_text(
            profit_summary_text(user_id),
            reply_markup=back_btn,
            parse_mode="Markdown"
        )
        return

    if query.data == "country_rename":
        set_pending_action(user_id, "set_country_name", {"first_time": False})
        await query.edit_message_text("✏️ اسم جدید کشورت رو بنویس و بفرست:")
        return
...
async def text_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- ورودی‌های متنی غیرعددی (اسم کشور) ----
    if pending["action"] == "set_country_name":
        if len(text) < 2 or len(text) > 30:
            await update.message.reply_text("⚠️ اسم باید بین ۲ تا ۳۰ کاراکتر باشه. دوباره بفرست:")
            return
        is_first_time = pending["data"].get("first_time", False)
        clear_pending_action(user_id)
        conn = get_db()
        conn.execute("UPDATE players SET country_name=? WHERE user_id=?", (text, user_id))
        conn.commit()
        conn.close()
        if is_first_time:
            await update.message.reply_text(
                f"✅ کشورت «{text}» نام‌گذاری شد!\n\nبرای شروع، از منوی زیر استفاده کن 👇",
                reply_markup=main_menu_keyboard(user_id)
            )
        else:
            await update.message.reply_text(
                f"✅ اسم کشورت به «{text}» تغییر کرد.",
                reply_markup=main_menu_keyboard(user_id)
            )
        return
...
def get_owned_quantity(user_id: int, category: str, item_code: str = None) -> float:
    """موجودی فعلی بازیکن از یه دسته/آیتم خاص - برای نمایش قبل از سوال «چند تا؟»"""
    conn = get_db()
    if category == "food":
        res = conn.execute("SELECT food FROM resources WHERE user_id=?", (user_id,)).fetchone()
        conn.close()
        return res["food"] if res else 0
    if category in RESOURCE_FIELD_MAP:
        field = RESOURCE_FIELD_MAP[category][0]
        res = conn.execute(f"SELECT {field} FROM resources WHERE user_id=?", (user_id,)).fetchone()
        conn.close()
        return res[field] if res else 0
    # نظامی
    if item_code:
        u = conn.execute("SELECT quantity FROM military_units WHERE user_id=? AND item_code=?",
                          (user_id, item_code)).fetchone()
    else:
        u = conn.execute("SELECT SUM(quantity) as quantity FROM military_units WHERE user_id=? AND item_type=?",
                          (user_id, category)).fetchone()
    conn.close()
    return (u["quantity"] or 0) if u else 0
...
def compute_max_admin_resource_buy(user_id: int, resource_type: str) -> int:
    price = int(get_setting(f"admin_shop_{resource_type}_price") or 10)
    field, cap_field = RESOURCE_FIELD_MAP[resource_type]
    conn = get_db()
    p = conn.execute("SELECT gold FROM players WHERE user_id=?", (user_id,)).fetchone()
    ...
    conn.close()
    space_left = res[cap_field] - res[field]
    max_by_gold = (p["gold"] // price) if price > 0 else space_left
    return max(0, int(min(max_by_gold, space_left)))


def compute_max_cap_metal_buy(user_id: int) -> int:
    ...
    max_by_cap = (p["cap_points"] // price) if price > 0 else space_left
    return max(0, int(min(max_by_cap, space_left)))
...
async def shops_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("shopres_max_"):
        resource_type = data.replace("shopres_max_", "")
        max_qty = compute_max_admin_resource_buy(user_id, resource_type)
        if max_qty <= 0:
            await safe_answer(query, "الان نه پول کافی داری نه جای انبار خالی.", show_alert=True)
            return
        success, error = buy_admin_resource(user_id, resource_type, max_qty)
        if success:
            await query.edit_message_text(
                f"✅ {max_qty} {RESOURCE_LABELS[resource_type]} خریداری شد (حداکثر ممکن).",
                reply_markup=shops_menu_keyboard()
            )
        else:
            await safe_answer(query, error, show_alert=True)
    ...
    if data.startswith("shopres_"):
        ...
        space_left = res[cap_field] - res[field]
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔢 تعداد دلخواه", callback_data=f"shopres_custom_{resource_type}")],
            [InlineKeyboardButton("⬆️ خرید حداکثر", callback_data=f"shopres_max_{resource_type}")],
            [InlineKeyboardButton("🔙 بازگشت", callback_data="shop_admin")],
        ])
        await query.edit_message_text(
            f"🛒 {RESOURCE_LABELS[resource_type]}\n\n"
            f"قیمت هر واحد: {price} پول\n"
            f"موجودی فعلی: {owned:.0f} | فضای خالی انبار: {space_left:.0f}",
            reply_markup=kb
        )
        return
    ...
    if data == "capshop_metal_max":
        max_qty = compute_max_cap_metal_buy(user_id)
        if max_qty <= 0:
            await safe_answer(query, "الان نه کاپ کافی داری نه جای انبار خالی.", show_alert=True)
            return
        success, error = buy_cap_metal(user_id, max_qty)
        if success:
            await query.edit_message_text(f"✅ {max_qty} فلز خریداری شد (حداکثر ممکن).",
                                           reply_markup=shops_menu_keyboard())
        else:
            await safe_answer(query, error, show_alert=True)
        return

    if data == "capshop_metal_custom":
        set_pending_action(user_id, "buy_cap_metal", {})
        await query.edit_message_text("چند واحد فلز می‌خوای بخری؟ عدد رو تایپ کن.")
        return

    if data == "capshop_metal":
        price = get_setting("cap_shop_metal_price")
        owned = get_owned_quantity(user_id, "metal")
        conn = get_db()
        res = conn.execute("SELECT metal, metal_cap FROM resources WHERE user_id=?", (user_id,)).fetchone()
        conn.close()
        space_left = res["metal_cap"] - res["metal"]
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔢 تعداد دلخواه", callback_data="capshop_metal_custom")],
            [InlineKeyboardButton("⬆️ خرید حداکثر", callback_data="capshop_metal_max")],
            [InlineKeyboardButton("🔙 بازگشت", callback_data="shop_cap")],
        ])
        await query.edit_message_text(
            f"⚙️ فلز\n\nقیمت هر واحد: {price} کاپ\n"
            f"موجودی فعلی: {owned:.0f} | فضای خالی انبار: {space_left:.0f}",
            reply_markup=kb
        )
        return
    ...
    if data.startswith("marketsell_cat_"):
        ...
        if category in ("tank", "jet", "ship", "missile"):
            ...
            owned = get_owned_quantity(user_id, category)
            set_pending_action(user_id, "market_sell_quantity", {"category": category, "item_code": None})
            await query.edit_message_text(
                f"چند تا می‌خوای بفروشی؟ (موجودی فعلی: {owned:.0f})\nعدد رو تایپ کن."
            )
        return
    ...
    if data.startswith("marketsell_item_"):
        ...
        conn = get_db()
        u = conn.execute("SELECT item_type, quantity FROM military_units WHERE user_id=? AND item_code=?",
                          (user_id, item_code)).fetchone()
        conn.close()
        set_pending_action(user_id, "market_sell_quantity", {"category": u["item_type"], "item_code": item_code})
        await query.edit_message_text(
            f"چند تا می‌خوای بفروشی؟ (موجودی فعلی: {u['quantity']:.0f})\nعدد رو تایپ کن."
        )
        return
    ...
    if data.startswith("instsell_"):
        ...
        if "_" in rest:
            ...
            owned = get_owned_quantity(user_id, item_type, item_code)
        else:
            set_pending_action(user_id, "instant_sell_quantity", {"category": rest, "item_code": None})
            owned = get_owned_quantity(user_id, rest)
        unit_value = get_instant_sell_value(rest.split("_")[0])
        await query.edit_message_text(
            f"چند تا می‌خوای بفروشی؟ (موجودی فعلی: {owned:.0f} | ارزش هر واحد: {unit_value:.1f} پول)\n"
            f"عدد رو تایپ کن."
        )
        return
...
SETTINGS_CATEGORIES = {
    "build": ("🏗️ ساخت‌وساز", [
        ...
        ("mine_metal_build_gold", "ساخت معدن فلز (پول)"), ("mine_metal_build_fertilizer", "ساخت معدن فلز (کود)"),
        ("mine_fertilizer_build_gold", "ساخت معدن کود (پول)"),
        ("mine_oil_build_gold", "ساخت معدن نفت (پول)"),
        ...
        ("mine_upgrade_base_gold", "ارتقای معدن نفت/کود/فلز - پایه (پول)"),
        ("mine_upgrade_base_metal", "ارتقای معدن نفت/کود/فلز - پایه (فلز)"),
        ("farm_upgrade_base_gold", "ارتقای مزرعه - پایه (پول)"),
        ("farm_upgrade_base_fertilizer", "ارتقای مزرعه - پایه (کود)"),
    ]),
    "starter": ("🎁 سرمایه اولیه و جمعیت", [
        ("starter_gold", "پول اولیه"), ("starter_oil", "نفت اولیه"),
        ("starter_metal", "فلز اولیه"), ("starter_fertilizer", "کود اولیه"),
        ("starter_food", "غذای اولیه"), ("starter_electricity_civil", "برق اولیه"),
        ("min_population_floor", "حداقل جمعیت باقی‌مونده (هیچ‌وقت پایین‌تر نمیره)"),
        ("starvation_loss_rate_per_hour", "نرخ تلفات گرسنگی در ساعت"),
        ("factory_permanent_worker_bonus", "نیروی رایگان دائمی هر کارخونه"),
    ]),
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex(f"^{START_BUTTON_TEXT}$"), start_command))
    app.add_handler(CallbackQueryHandler(main_menu_callback, pattern="^(menu_|help_sec_|help_admin$)"))
    ...
    # فاز ۶: اتحاد
    alliance_patterns = "^(alliance_|giftpick_|giftres_|contribres_|kickpick_|announce_mode_)"
