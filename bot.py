...
MAX_BATCH_HOURS = 40  # سقف زمانی برای محاسبه حداکثر تعداد قابل ساخت یک‌جا
...
# فقط آیتم‌های خام پایه که هیچ زنجیره تولیدی ندارن مستقیم قابل خریدن
BUY_CATALOG = {
    "ارد":  {"emoji": "⬜"},
    "شکر":  {"emoji": "⬜"},
}
...
# وضعیت موقت پیام‌هایی که منتظر «ریپلای با یه عدد» هستن (در حافظه)
PENDING_ACTIONS = {}
...
def execute_factory_start(user_id: int, building_key: str, qty: int):
    ...
    qty = min(qty, max_qty)
    if qty <= 0:
        return False, f"مواد اولیه کافی نداری."
...
# ------------------------------------------------------------------
# سیستم تعداد دلخواه: دکمه‌های درصد + امکان ریپلای با یه عدد دلخواه
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# ناوبری: تولید (منوی اصلی) → دسته (لیست ساختمون‌های من) → پنل اختصاصی هر ساختمون
# ------------------------------------------------------------------
...
def build_livestock_building_panel(user_id: int, key: str):
    ...
    lines = [
        ...
        "",
        f"محصولات جمع‌آوری‌شده تا کنون :",
...
def build_inventory_text(user_id: int):
    ...
    player = get_player(user_id)
    used = sum(inv.values())
    lines = [f"انباری کاربر :", "", f"سطح : —", f"ظرفیت : {fnum(used)}/{fnum(player['storage_capacity'])}", f"موجودی : {fnum(player['coins'])} 💰", "", "ایتم‌ها 📦:"]
...
GUIDE_TEXT = (
    ...
    "🏭 کارخونه — هر محصول فرعی (نان، پنیر، کیک، پیتزا، سوسیس، هات‌داگ، فرنچ، لباس، نیمرو، پارچه، ماست...) کارخونه اختصاصی خودشو لازم داره. ورودیش فقط از تولید خودته.\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ریپلای به یه پیام «چند تا می‌خوای؟»
