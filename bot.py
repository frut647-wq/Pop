...
def submit_attack(attacker_id: int, defender_id: int, cart: list, existing_war_id: int = None):
    """حمله رو نهایی می‌کنه: واحدها مصرف می‌شن، دمیج محاسبه و بعد از کسر پدافند، نتیجه فوری اعمال می‌شه.
    اگه existing_war_id داده بشه (حمله‌ای که قبلاً منتظر تایید ادمین بود)، همون ردیف رو آپدیت می‌کنه به‌جای ساخت ردیف جدید.
    ...
    if existing_war_id:
        conn.execute("""
            UPDATE war_regular SET status='finished', total_damage=?, defense_blocked=?,
                                    start_time=?, end_time=?, result=?
            WHERE id=?
        """, (round(raw_damage, 1), round(blocked_damage, 1), now, end_time, result, existing_war_id))
...
def approve_pending_attack(war_id: int):
    ...
    result = submit_attack(war["attacker_id"], war["defender_id"], cart, existing_war_id=war_id)
    if result is None:
        conn = get_db()
...
async def attack_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("attack_target_"):
        ...
        # اعلامیه خودکار به کانال (فاز ۱۱)
        await post_war_announcement(context.bot, user_id, defender_id, result)
        return
...
def initiate_alliance_war(leader_id: int, defender_alliance_id: int):
    ...
    # ترتیب اهداف: ضعیف‌ترین (کمترین اچ‌پی) اول
    order = sorted([d["user_id"] for d in defenders], key=lambda uid: get_country_hp(uid))
...
def start_alliance_war(war_id: int):
    """فقط ادمین صدا می‌زنه (فعلاً با دستور، تا پنل ادمین فاز ۱۲ ساخته بشه)"""
    conn = get_db()
    war = conn.execute("SELECT * FROM war_alliance WHERE id=?", (war_id,)).fetchone()
    ...
    # فوری حل می‌شه (تا فاز زمان‌بند/APScheduler اضافه بشه)
    return resolve_alliance_war(war_id)
...
async def start_alliance_war_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = start_alliance_war(war_id)
    if isinstance(result, tuple):
        await update.message.reply_text(f"❌ {result[1]}")
    else:
        await update.message.reply_text(
            f"✅ جنگ شروع و نتیجه‌گیری شد!\n"
            f"کشورهای نابودشده: {result['destroyed_count']}\n"
            f"کاپ اضافه‌شده به گاوصندوق: {result['cap_earned']}"
        )
        await post_alliance_war_announcement(context.bot, war_id, "finished")
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("admin_approveatk_"):
        ...
        if result:
            await query.edit_message_text(
                f"✅ حمله تایید و اجرا شد.\nنتیجه: {result['result']}\nدمیج مؤثر: {result['effective_damage']}",
                reply_markup=admin_pending_attacks_keyboard()
            )
            war = get_db().execute("SELECT attacker_id, defender_id FROM war_regular WHERE id=?", (war_id,)).fetchone()
            try:
                await context.bot.send_message(war["attacker_id"], "✅ ادمین حمله‌ت رو تایید کرد و اجرا شد!")
                await context.bot.send_message(war["defender_id"], "🚨 کشور شما مورد حمله (تایید ادمین) قرار گرفت!")
            except Exception:
                pass
        else:
            await safe_answer(query, error, show_alert=True)
